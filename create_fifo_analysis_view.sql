-- FIFO Analysis System with Old vs New Batch Tracking
-- This creates views and triggers to properly track inventory batches
...
-- 5. Create triggers to auto-sync product quantities
DELIMITER $$
...
FOR EACH ROW
BEGIN
    -- Update product total when new FIFO stock is added
    CALL UpdateProductTotalFromFIFO(NEW.product_id);
END$$
...
FOR EACH ROW
BEGIN
    -- Update product total when FIFO stock is modified
    CALL UpdateProductTotalFromFIFO(NEW.product_id);
END$$
...
FOR EACH ROW
BEGIN
    -- Update product total when FIFO stock is removed
    CALL UpdateProductTotalFromFIFO(OLD.product_id);
END$$
...
SELECT * FROM v_fifo_batch_analysis WHERE product_id = 217 LIMIT 5;

SELECT 'PRODUCT FIFO SUMMARY SAMPLE' as info;
SELECT * FROM v_product_fifo_summary WHERE product_id = 217;

SELECT 'OLD VS NEW BREAKDOWN SAMPLE' as info;
SELECT * FROM v_fifo_old_vs_new WHERE product_id = 217;

SELECT 'TRANSFER RECOMMENDATIONS SAMPLE' as info;
SELECT * FROM v_fifo_transfer_recommendations WHERE product_id = 217 LIMIT 3; 